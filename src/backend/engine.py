...
class ProcessingSession:
    ...
    def __init__(self, session_id: str, session_config: dict, app_config: dict):
        ...
        Parameters
        ----------
        session_config : per-request config (model, tracker, classes, roi…)
        app_config     : parsed config.yaml (used for labels lookup, defaults)
        """
        self.session_id = session_id

        # COCO labels come from config.yaml → classes.labels
        ...
        # vehicle_counts keyed by labels for classes actually used in this session
        ...
        # Session meta — fall back to config.yaml detection/tracking defaults
...
class VehicleCountingEngine:
    # Streaming / perf constants (not in config.yaml — pure implementation detail)
    _FPS_WINDOW   = 30
    _JPEG_QUALITY = 70
    ...
    def __init__(self, app_config: dict):
        ...
        ----------
        app_config : dict
            Parsed content of config.yaml (passed in from app entry-point).
        """
        self.app_config = app_config

        # Paths come from config.yaml → system
    ...
    def start_session(self, config: dict) -> str:
        ...
        target = self._run_video if mode == "upload" else self._run_camera
    ...
    def _build_counter(self, session: ProcessingSession, frame_shape: tuple, config: dict) -> ObjectCounter:
        """Build and return a configured ObjectCounter instance."""
    ...
    def _init_source(
        ...
        """
        Read the first frame, build the counter, and open the VideoWriter.
        Returns (counter, writer) on success, or (None, None) and sets
        session.status = "stopped" on failure.
        """
    ...
    def _process_frame(
        ...
        """
        Run ObjectCounter on one frame, update session stats,
        write to output video, encode for MJPEG stream,
        update FPS rolling average and resource usage.
        """
        # --- Object counting ---
        try:
            ...
            source_tag = "Frame" if is_video else "Camera frame"
            print(f"[engine] {source_tag} processing error: {e}")
            annotated = frame

        # --- Write & stream ---
        writer.write(annotated)
        _, buf = cv2.imencode(
            ".jpg", annotated, [cv2.IMWRITE_JPEG_QUALITY, self._JPEG_QUALITY]
        )
        session.latest_frame = buf.tobytes()
        session.frame_count += 1

        # --- Progress & ETA (video only) ---
        if is_video:
            session.processed_frames = session.frame_count
            if session.total_frames > 0:
                session.progress = int(
                    session.processed_frames * 100 / session.total_frames
                )
        ...
        if len(frame_times) > self._FPS_WINDOW:
            frame_times.pop(0)
        session.fps = (
            1.0 / (sum(frame_times) / len(frame_times)) if frame_times else 0.0
        )
    ...
    def _run_video(self, session: ProcessingSession, config: dict) -> None:
        ...
        cap.set(cv2.CAP_PROP_POS_FRAMES, 0)  # rewind past first frame
    ...
    def _finalize_session(self, session: ProcessingSession, config: dict) -> None:
        ...
        summary = {
            ...
            "confidence":              config.get("confidence", self.app_config.get("detection", {}).get("confidence", 0.25)),
            "iou":                     config.get("iou",        self.app_config.get("detection", {}).get("iou",        0.7)),
